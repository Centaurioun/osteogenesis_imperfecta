    ...
    if os.path.isdir(input_path):
        for value in REQ.values():
            if not os.path.exists(os.path.join(input_path, value)):
                fail(f"Missing required file in directory: {value}")
        return (
            read_csv_from_dir(input_path, REQ['t1']),
            read_csv_from_dir(input_path, REQ['t2']),
            read_csv_from_dir(input_path, REQ['t3']),
            read_csv_from_dir(input_path, REQ['rob']),
            read_csv_from_dir(input_path, REQ['cv']),
        )

    if zipfile.is_zipfile(input_path):
        with zipfile.ZipFile(input_path) as zf:
            for value in REQ.values():
                if value not in zf.namelist():
                    fail(f"Missing required file in zip: {value}")
            return (
                read_csv_from_zip(zf, REQ['t1']),
                read_csv_from_zip(zf, REQ['t2']),
                read_csv_from_zip(zf, REQ['t3']),
                read_csv_from_zip(zf, REQ['rob']),
                read_csv_from_zip(zf, REQ['cv']),
            )

    fail('Input must be either a directory containing FINAL CSV files or a valid zip archive.')
...
def fig_cv(cv: pd.DataFrame, outdir: str):
    ...
    for i,ep in enumerate(order):
        for j,meth in enumerate(['LOO','RSKF']):
            ...
            # Bootstrap CI might not cleanly surround the point estimate (delta_auc) due to skew.
            err_low = max(0, x[-1] - lo)
            err_high = max(0, hi - x[-1])
            xerr.append([err_low, err_high])
...
def main():
    ...
    ap.add_argument('--input', required=True, help='Path to a directory of FINAL CSV files or to outputs_FINAL_1_2.zip')
