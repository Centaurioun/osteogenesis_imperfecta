...
> **Arşiv notu / non-authoritative warning:** Bu dosya tarihsel AI oturum bağlamını korumak için saklanmıştır. İçinde geçen `v3`, `outputs_v3`, `oi_oro_dental_master_v3.ipynb` ve `issue_log_v3.csv` referansları provenance amaçlıdır. `Manuscript_Data` paketi içinde authoritative final analysis branch **FINAL.1.2**’dir; güncel başlangıç için önce `06_ai_handoff_context/FINAL_HANDOFF_QUICKSTART.md` ve `README_Manuscript_Data.md` okunmalıdır.
