...
def figA_prevalans(t1: pd.DataFrame, outdir: str):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "FigA_prevalans_TR.pdf"))
...
def figB_gen_grup(t2: pd.DataFrame, outdir: str):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "FigB_gen_grup_TR.pdf"))
...
def figC_inferans_ozet(t3: pd.DataFrame, outdir: str):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "FigC_inferans_ozet_TR.pdf"))
...
def figE_robustluk(rob: pd.DataFrame, outdir: str):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "FigE_robustluk_TR.pdf"))
...
def figF_cv_delta_auc(cv: pd.DataFrame, outdir: str):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "FigF_cv_delta_auc_TR.pdf"))
    plt.savefig(os.path.join(outdir, "FigF_cv_delta_auc_TR.png"))
    plt.close()

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--input", required=True, help="outputs_FINAL_1_2.zip yolu")
    ...
    with zipfile.ZipFile(args.input) as zf:
        ...
        for v in REQ_FILES.values():
            if v not in names:
                fail(f"Zip içinde eksik dosya: {v}")

        t1 = read_csv_from_zip(zf, REQ_FILES["t1"])
        t2 = read_csv_from_zip(zf, REQ_FILES["t2"])
        t3 = read_csv_from_zip(zf, REQ_FILES["t3"])
        rob = read_csv_from_zip(zf, REQ_FILES["rob"])
        cv = read_csv_from_zip(zf, REQ_FILES["cv"])
    ...
    print("OK — Türkçe figürler üretildi (PDF+PNG).")
