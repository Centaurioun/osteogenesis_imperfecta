...
loo_pattern = r"(try:\s+auc_a, auc_g = roc_auc_score\(y, preds_a\), roc_auc_score\(y, preds_g\)\s+ci_l, ci_h, has_warn = get_auc_ci\(y, preds_a, preds_g, n_boot=manifest\[\"bootstrap_iters\"\]\)\s+warns = \[\]\s+if auc_a < 0\.5 or auc_g < 0\.5: warns\.append\(\"AUC < 0\.5\"\)\s+if has_warn: warns\.append\(\"Some bootstrap samples had < 2 classes\"\)\s+cv_rows\.append\(\{.*?\}\))"
...
text = re.sub(loo_pattern, new_loo_str, text, flags=re.DOTALL)


rskf_pattern = r"(if aucs_a and aucs_g:\s+auc_a, auc_g = np\.mean\(aucs_a\), np\.mean\(aucs_g\)\s+p_a_mean, p_g_mean = oof_a\.mean\(axis=0\), oof_g\.mean\(axis=0\)\s+ci_l, ci_h, has_warn = get_auc_ci\(y, p_a_mean, p_g_mean, n_boot=manifest\[\"bootstrap_iters\"\]\)\s+warns = \[\]\s+if auc_a < 0\.5 or auc_g < 0\.5: warns\.append\(\"AUC < 0\.5\"\)\s+if has_warn: warns\.append\(\"Some bootstrap samples had < 2 classes\"\)\s+cv_rows\.append\(\{.*?\}\))"

new_rskf_str = """if aucs_a and aucs_g:
                    n_pos, n_neg = np.sum(y == 1), np.sum(y == 0)
...
text = re.sub(rskf_pattern, new_rskf_str, text, flags=re.DOTALL)
