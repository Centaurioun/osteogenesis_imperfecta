...
+        PACKAGE_RUN = ROOT / "{latest_run_rel}"
+
+        required_paths = [
...
+
+    scaffold_signatures = ["nbf.v4.new_notebook()", "target_notebook_path", "with target_notebook_path.open", "example_code ="]
...
+
+    execution_missing = [idx for idx, cell in enumerate(code_cells, start=1) if cell.get("execution_count") is None]
+    output_free_required = [num for num in substantive_code_cell_numbers if len(cells[num - 1].get("outputs", [])) == 0]
+
+    source_map_df = pd.read_csv(source_map_path)
+    required_blocks = {"overall_descriptive_results", "gene_group_results", "inferential_results", "robustness_results", "cv_internal_verification", "figure_display"}
+    missing_blocks = sorted(required_blocks - set(source_map_df["analysis_block"]))
+
+    validation_sections = [
+        ("Structure checks", [
...
+
+    validation_text = [f"# oi_oro_dental_consolidated_v{version} validation", "", f"Overall status: **{overall_status}**", ""]
...
+        f"- residual_caveats_or_blockers: `{'None' if passed else 'Validation failures present; see validation.md'}`",
+        "",
+        "## Cell plan",
+        "",
+    ]
+    for row in build_cell_plan():
+        execution_summary_lines.append(f"- Cell {row['cell_no']} [{row['cell_type']}] {row['section_name']} :: {row['purpose']} :: outputs={row['expected_outputs']}")
+
...
+
+    build_source_map(version, latest_run).to_csv(source_map_path, index=False)
...
+    validation_text, execution_summary_text = validate_notebook(notebook_path, source_map_path, version, latest_run, execution_start, execution_end)
